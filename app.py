...
NUM_CHUNKS_TO_FETCH_SEMANTICALLY = 5 # How many to initially FETCH for semantic search 
...
def process_document_with_docling(file_bytes, filename):
    if file_bytes is None:
        st.error("Docling Processing: No file bytes received.")
        return None
...
if not st.session_state.get('show_summary', False) and \
   ...
   not st.session_state.get('in_heatmap_quiz_mode', False):
    ...
    # FIX: Safe Bytes Caching applied here
    if uploaded_file_widget_result is not None:
        st.session_state.uploaded_file_bytes = uploaded_file_widget_result.read()
        st.session_state.uploaded_file_name = uploaded_file_widget_result.name
...
if uploaded_file is not None and not st.session_state.get('in_heatmap_quiz_mode', False): 
    ...
    if needs_full_processing:
        ...
        # FIX: Process with bytes
        docling_output_list = process_document_with_docling(st.session_state.uploaded_file_bytes, st.session_state.uploaded_file_name)
    ...
    if st.session_state.get('substantive_chunks_for_quiz') is None:
        if st.session_state.get('_chunks_backup') is not None:
            ...
            with st.spinner("Reprocessing document to recover chunks..."):
                docling_output = process_document_with_docling(
                ...
                if docling_output:
                    ...
                    st.error("Could not recover document chunks. Please re-upload.")
                    st.session_state.in_heatmap_quiz_mode = False
                    st.rerun()
        else:
            st.error("No document data found. Please upload a file first.")
            st.session_state.in_heatmap_quiz_mode = False
            st.rerun()
