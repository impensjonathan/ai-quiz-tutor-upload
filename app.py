            ...
            if st.session_state.show_explanation:
                ...
                if st.session_state.last_answer_correct:
                    def _back_to_summary():
                        ...
                        st.session_state.show_summary = True
                        st.session_state.heatmap_quiz_last_answer_incorrect = False
                    st.button("Back to Quiz Summary", on_click=_back_to_summary)
                else: 
                    def _retry_focused():
                        st.session_state.current_question_data = None 
                        st.session_state.show_explanation = False
                    st.button("Try Another Question on this Topic", on_click=_retry_focused)
            
            st.divider()
            def _end_focused_quiz():
                st.session_state.in_heatmap_quiz_mode = False
                st.session_state.heatmap_quiz_source_chunk_idx = None
                st.session_state.current_question_data = None 
                st.session_state.show_summary = True
            st.button("End Focused Quiz & View Summary", on_click=_end_focused_quiz)

def show_summary_mode():
    st.title("Quiz Summary")
    st.caption(f"Document: {st.session_state.get('uploaded_file_name', 'Unknown')}")
    ...
    if st.session_state.get('show_heatmap_chunk_detail', False) and st.session_state.get('selected_heatmap_chunk_index') is not None:
        ...
        if 0 <= idx < len(doc_chunk_details):
            ...
            with st.expander(f"Path: {path_title} (Paragraph {idx + 1})", expanded=True): 
                ...
                with col1_exp:
                    st.button("Quiz me on this chunk", key=f"q_btn_{idx}", on_click=_start_focused_quiz_cb, args=(idx,))
                with col2_exp:
                    st.button("Close Detail", key=f"c_btn_{idx}", on_click=_close_detail_cb)
    ...
    st.button("Start New Quiz Once More", on_click=_restart_quiz)
...
def show_ready_screen():
    ...
    if st.button("Start Quiz!", type="primary", on_click=_start_quiz_main): 
    ...
    with st.container(border=True):
        if st.session_state.current_question_data:
            ...
            def _stop_quiz():
                st.session_state.show_summary = True
                st.session_state.quiz_started = False
            st.button("Stop Quiz", on_click=_stop_quiz)
...
def handle_file_upload_and_processing():
    ...
    if uploaded_file:
        ...
        current_file_key = f"{st.session_state.uploaded_file_name}_{st.session_state.uploaded_file_size}"
        needs_full_processing = False
        
        if st.session_state.get('uploaded_file_key') != current_file_key:
            needs_full_processing = True
        elif st.session_state.get('substantive_chunks_for_quiz') is None or not st.session_state.get('vector_store_setup_done', False):
            needs_full_processing = True
            ...
            with st.spinner("Processing document logic..."):
                ...
                if docling_output:
                    ...
                    if st.session_state.llm_configured:
                        setup_vector_store(st.session_state.substantive_chunks_for_quiz, st.secrets["GEMINI_API_KEY"], st.session_state.uploaded_file_name)
...
if st.session_state.get('in_heatmap_quiz_mode') or st.session_state.get('show_summary') or st.session_state.get('quiz_started'):
    if not st.session_state.get('substantive_chunks_for_quiz'):
        if st.session_state.get('_chunks_backup'):
...
has_chunks = bool(st.session_state.get('substantive_chunks_for_quiz'))
